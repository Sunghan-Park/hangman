// 화면 요소 초기화
export const initializeDisplay = (instructions, title, start, category, answerLetters, alphabets) => {
...
export const selectRandomWord = (categoryList, category) => {
  ...
  const randomList = categoryList[randomCategoryKey][
...
// 다시 플레이 함수
export const restartGame = (instructions, title, start, category, answerLetters, alphabets, gameover, myWord) => {
  // 게임오버 화면 숨기기
...
// 시작 화면으로 돌아가기 함수
export const goToStartPage = (instructions, title, start, category, answerLetters, alphabets, gameover, myWord, timer, timerCount) => {
  // 게임오버 화면 숨기기
  gameover.forEach(elem => elem.classList.add('display-none'));
...
// 타이머 시작
export const startTimer = (timer, timerCount, checkGameOver) => {
  timerCount = 60;
